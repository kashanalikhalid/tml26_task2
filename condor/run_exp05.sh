#!/usr/bin/env bash
# Wrapper for experiment 05: PGD strong adversarial transfer.
# Outputs go to outputs/exp05/{features,submission}.csv on NFS.
...
mkdir -p "$SCRATCH/cifar100" outputs/exp05
echo "::: [exp05] host=$(hostname) scratch=$SCRATCH"
df -h "$SCRATCH" | head -2
...
# Same cache-check / scratch-fallback pattern as run_score.sh
nfs_cache_complete() {
...
$PY -u experiments/exp05_pgd_strong.py \
    ...
    --features-out outputs/exp05/features.csv \
    --submission-out outputs/exp05/submission.csv \
    --batch-size "${BATCH_SIZE:-256}"

echo "::: [exp05] outputs:"
ls -la outputs/exp05/
head -3 outputs/exp05/submission.csv
wc -l outputs/exp05/submission.csv
