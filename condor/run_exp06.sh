#!/usr/bin/env bash
# Wrapper for experiment 06: penultimate-layer CKA.
# Outputs go to outputs/exp06/{features,submission}.csv on NFS.
...
mkdir -p "$SCRATCH/cifar100" outputs/exp06
echo "::: [exp06] host=$(hostname) scratch=$SCRATCH"
df -h "$SCRATCH" | head -2
...
$PY -u experiments/exp06_cka.py \
    ...
    --features-out outputs/exp06/features.csv \
    --submission-out outputs/exp06/submission.csv \
    --batch-size "${BATCH_SIZE:-256}"

echo "::: [exp06] outputs:"
ls -la outputs/exp06/
head -3 outputs/exp06/submission.csv
wc -l outputs/exp06/submission.csv
