#!/usr/bin/env bash
# Wrapper for exp09_aug. Outputs -> outputs/exp09/.
set -euo pipefail
...
mkdir -p "$SCRATCH/cifar100" "$SCRATCH/cifar10" "outputs/exp09"
echo "::: [exp09] host=$(hostname) scratch=$SCRATCH"
df -h "$SCRATCH" | head -2
...
EXTRA_ARGS=""
if [ "exp09_aug" = "exp08_ood" ]; then
    EXTRA_ARGS="--cifar10-root $SCRATCH/cifar10"
fi

$PY -u experiments/exp09_aug.py \
    ...
    --features-out outputs/exp09/features.csv \
    --submission-out outputs/exp09/submission.csv \
    --batch-size "${BATCH_SIZE:-256}" $EXTRA_ARGS

echo "::: [exp09] outputs:"
ls -la outputs/exp09/
head -3 outputs/exp09/submission.csv
wc -l outputs/exp09/submission.csv
