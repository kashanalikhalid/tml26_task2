...
mkdir -p "$SCRATCH/cifar100" outputs/exp16
echo "::: [exp16] host=$(hostname)"
...
# Use NFS-cached CIFAR-100 (avoid the download hang)
if [ -d "$DATA_ROOT/cifar100" ]; then cp -r "$DATA_ROOT/cifar100" "$SCRATCH/cifar100"; fi
$PY -u experiments/exp16_multilayer_act.py \
    ...
    --features-out outputs/exp16/features.csv \
    --submission-out outputs/exp16/submission.csv
echo "::: done"; ls -la outputs/exp16/
