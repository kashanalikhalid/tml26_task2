...
if [ -s "outputs/shadow_stats/shadow_26.npz" ]; then
    echo "::: [shadow_26] stats file already exists, skipping training"
    exit 0
fi

SCRATCH="${_CONDOR_SCRATCH_DIR:-${TMPDIR:-/tmp}/tml26_$$}"
mkdir -p outputs/shadow_stats
echo "::: [shadow_26] host=$(hostname) scratch=$SCRATCH"
...
SCRATCH_STATS="$SCRATCH/shadow_26.npz"
$PY -u experiments/exp13_shadow_train.py \
    --stats-out "$SCRATCH_STATS" --seed 26 \
...
cp "$SCRATCH_STATS" "outputs/shadow_stats/shadow_26.npz"
ls -la outputs/shadow_stats/shadow_26.npz
