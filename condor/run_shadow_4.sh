...
if [ -s "outputs/shadow_stats/shadow_4.npz" ]; then
    echo "::: [shadow_4] stats file already exists, skipping training"
    exit 0
fi
...
SCRATCH_STATS="$SCRATCH/shadow_4.npz"
$PY -u experiments/exp13_shadow_train.py \
    --stats-out "$SCRATCH_STATS" --seed 4 \
...
cp "$SCRATCH_STATS" "outputs/shadow_stats/shadow_4.npz"
ls -la outputs/shadow_stats/shadow_4.npz
