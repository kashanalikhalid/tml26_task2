#!/usr/bin/env python3
"""Experiment 10 — Multi-seed PGD ensemble.

Hypothesis: PGD's random initialization adds variance to the adversarial
transfer rate per suspect. Running PGD with 3 different seeds and
averaging the per-suspect transfer scores reduces variance and gives a
more stable fingerprint.

Each seed crafts a slightly different set of adversaries from target;
the union of "where target's boundary breaks" is a more complete picture
of target's decision-boundary signature.
"""
...
from detect.adversarial import _predict as predict_argmax, adversarial_transfer_features, craft_pgd_adversaries  # noqa: E402
...
OUT_DIR = Path("outputs/exp10")
N_SEEDS = 3
ADV_EPS, ADV_ALPHA, ADV_STEPS, ADV_N = 8/255, 2/255, 10, 2000
...
SIGNS = {
    ...
    "adv_transfer_class_match_mean": +1, "adv_transfer_fooled_any_mean": +1,
}
...
def main():
    ...
    logging.info("[exp10] device=%s n_seeds=%d eps=%.4f", device, N_SEEDS, ADV_EPS)
    ...
    # Craft 3 sets of adversaries with different seeds
    ...
    for seed in range(N_SEEDS):
        ...
        t0=time.time()
        ax = craft_pgd_adversaries(target, adv_x_clean, adv_y, epsilon=ADV_EPS, alpha=ADV_ALPHA, steps=ADV_STEPS, batch_size=args.batch_size, device=device)
        t_adv = predict_argmax(target, ax, args.batch_size, device)
        t_clean = predict_argmax(target, adv_x_clean, args.batch_size, device)
        succ = float((t_clean != t_adv).float().mean().item())
        logging.info("seed %d: PGD in %.1fs, attack_success=%.3f", seed, time.time()-t0, succ)
        adv_sets.append((ax, t_adv, t_clean))

    paths=sorted(args.suspects.glob("suspect_*.safetensors"))
    rows=[]
    for path in tqdm(paths, desc="exp10"):
        ...
        try:
            ...
            # Average adv features across N_SEEDS PGD runs
            cm_list, fa_list = [], []
            for (ax, t_adv, t_clean) in adv_sets:
                s_adv = predict_argmax(m, ax, args.batch_size, device)
                s_clean = predict_argmax(m, adv_x_clean, args.batch_size, device)
                f = adversarial_transfer_features(t_adv, t_clean, s_adv, s_clean, adv_y)
                cm_list.append(f["adv_transfer_class_match"]); fa_list.append(f["adv_transfer_fooled_any"])
            row["adv_transfer_class_match_mean"] = float(np.mean(cm_list))
            row["adv_transfer_fooled_any_mean"] = float(np.mean(fa_list))
            row["adv_transfer_class_match_std"] = float(np.std(cm_list))
    ...
    print("[exp10] top-15:"); print(sub.sort_values("score",ascending=False).head(15).to_string(index=False))
    return 0
