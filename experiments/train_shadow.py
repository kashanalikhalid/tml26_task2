#!/usr/bin/env python3
"""Train one shadow model with the target's recipe and save stats + weights."""
...
def main():
    ...
    p.add_argument("--weights-out", type=Path, default=None)
    ...
    logging.info("seed=%d device=%s epochs=%d", args.seed, device, args.epochs)
    ...
    for ep in range(args.epochs):
        ...
        if (ep + 1) % 5 == 0 or ep == args.epochs - 1:
            logging.info("epoch %d loss=%.4f time=%.1fs", ep + 1, total / max(n, 1), time.time() - t0)
    ...
    def probe(x, y):
        ...
        return (top1.cpu().numpy().astype(np.int32),
                conf.cpu().numpy().astype(np.float32),
                ce.cpu().numpy().astype(np.float32))

    test_top1, test_conf, test_loss = probe(test_x, test_y)
    mem_top1, mem_conf, mem_loss = probe(mem_x, mem_y)
    nm_top1, nm_conf, nm_loss = probe(nm_x, nm_y)
    ...
    logging.info("saved %s", args.stats_out)

    if args.weights_out is not None:
        from safetensors.torch import save_file
        args.weights_out.parent.mkdir(parents=True, exist_ok=True)
        save_file(model.state_dict(), str(args.weights_out))
        logging.info("saved %s", args.weights_out)
